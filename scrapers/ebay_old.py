...
class EbayOldScraper(BaseScraper):
        ...
        for order_id in order_ids:
            ...
            for order_box in self.find_elements(
                ...
                section_data_items = order_box.find_element(
                    By.CSS_SELECTOR,
                    "div.order-info div.section-data-items",
                )
    ...
    def load_or_scrape_order_ids(self):
        ...
        if not self.options.use_cached_orderlist or not self.can_read(
            ...
            while True:
                ...
                self.rand_sleep(2, 4)
            self.log.info(
                "Loaded %s new style order ids from eBay.com",
                len(order_ids),
            )
            self.log.debug(
            ...
            self.log.info(
                "Loaded %s new style order ids from json",
                len(order_ids),
            )
    ...
    def browser_website_switch_mode(self, switch_to_mode=None):
        ...
        to_classic_link = self.find_element(
            By.CSS_SELECTOR,
            "div.gh-mwebfooter__siteswitch a",
        )
        changed = False
